...
def main():
	...
	for file in sorted(Path.cwd().iterdir()):
		...
		if file.suffix == '.esx':
			...
			# count = sum(1 for ap in accessPointsJSON.get("data", []) if item.get(target_element) == "Alice")

			for ap in tqdm(accessPointsJSON['accessPoints'], desc="Processing APs"):  # Use tqdm here
				# print(ap['name'])
